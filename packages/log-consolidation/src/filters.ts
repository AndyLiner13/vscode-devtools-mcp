 ...
 */
export function applyFilters(result: ConsolidationResult, originalLines: string[], filters: FilterOptions): ConsolidationResult | undefined {
	const hasPostFilters = filters.templateId !== undefined || filters.severity !== undefined || filters.timeRange !== undefined || filters.minDuration !== undefined || filters.correlationId !== undefined || filters.includeStackFrames === false;
	...
	if (filters.minDuration) {
		...
		if (thresholdMs > 0) {
			templates = templates.filter((t) => t.durationSamples.some((d) => parseDuration(d) >= thresholdMs));
		}
	}
	...
	if (filters.correlationId) {
		...
		templates = templates.filter((t) => t.correlationIdSamples.some((id) => id.toLowerCase().includes(targetId)));
	}
 ...
 */
function expandTemplate(result: ConsolidationResult, originalLines: string[], templateId: string): ConsolidationResult | undefined {
	...
	if (matchingLines.length > maxLines) {
		...
		formatted = [`## Template ${templateId}: ${template.pattern}`, `**Occurrences:** ${template.occurrences} | **Severity:** ${template.severity}`, `**Lines ${startLine + 1}–${endLine + 1}:**\n`, ...matchingLines].join('\n');
	}
 ...
 */
function filterByTimeRange(templates: Template[], originalLines: string[], range: TimeRange): Template[] {
