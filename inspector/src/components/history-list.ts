...
async function renderHistory(): Promise<void> {
  ...
  let focusedRecordId: string | null = null;
  let focusedValue = '';
  ...
  if (focusedRecordId) {
    const input = container.querySelector(
      `input[data-record-id="${focusedRecordId}"]`,
...
function renderEntry(record: ExecutionRecord): HTMLElement {
  ...
  chevron.appendChild(
    createSvgIcon(isExpanded ? ICON_PATHS.chevronDown : ICON_PATHS.chevronRight),
  );
  ...
  commentField.addEventListener('click', (e) => e.stopPropagation());
