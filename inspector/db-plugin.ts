    ...
    configureServer(server) {
      ...
      async function handleApi(req: IncomingMessage, res: ServerResponse): Promise<void> {
        ...
        if (pathname === '/api/records' && method === 'GET') {
          ...
          if (tool) {
            const rows = db
              .prepare('SELECT * FROM records WHERE tool_name = ? ORDER BY priority ASC')
              .all(tool);
            json(res, rows.map(rowToApi));
          } else {
            const rows = db
              .prepare('SELECT * FROM records ORDER BY tool_name, priority ASC')
              .all();
        ...
        if (commentMatch && method === 'PATCH') {
          ...
          db.prepare('UPDATE records SET comment = ? WHERE id = ?').run(
