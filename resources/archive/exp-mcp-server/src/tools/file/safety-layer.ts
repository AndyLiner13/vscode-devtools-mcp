...
import type { AutoFix, DetectedIntent, FileEditResult, PropagatedChange, RemainingError } from './types.js';

import { fileApplyCodeAction, fileApplyEdit, fileExecuteRename, fileFindReferences, fileGetCodeActions, fileGetDiagnostics, fileGetSymbols, fileReadContent, fileShowEditDiff, type NativeDocumentSymbol } from '../../client-pipe.js';
 ...
 */
const HARMFUL_FIX_PATTERNS = [/change spelling/i, /did you mean/i];
 ...
 */
function findSymbolByQualifiedName(symbols: NativeDocumentSymbol[], qualifiedName: string): NativeDocumentSymbol | undefined {
 ...
 */
export async function executeEditWithSafetyLayer(filePath: string, startLine: number, endLine: number, newContent: string): Promise<FileEditResult> {
	...
	if ((renames.length > 0 || deletes.length > 0) && oldContent !== undefined) {
		...
		for (const del of deletes) {
			...
			try {
				const refs = await fileFindReferences(filePath, oldSym.selectionRange.startLine, oldSym.selectionRange.startChar);
				const normalizedFilePath = normalizePath(filePath);
				const externalRefs = refs.references.filter((r) => {
					const refPath = normalizePath(r.file);
					return refPath !== normalizedFilePath && !refPath.endsWith(normalizedFilePath) && !normalizedFilePath.endsWith(refPath);
				});
		...
		for (const renameIntent of renames) {
			...
			try {
				const renameResult = await fileExecuteRename(filePath, oldSym.selectionRange.startLine, oldSym.selectionRange.startChar, renameIntent.newName);
 ...
 */
async function finalize(filePath: string, detectedIntents: DetectedIntent[], propagated: PropagatedChange[], autoFixed: AutoFix[], editStartLine: number): Promise<FileEditResult> {
		...
		for (const error of newErrors) {
			...
			try {
				...
				if (preferred && !isHarmfulAutoFix(preferred.title, hasDeletes)) {
					const applyResult = await fileApplyCodeAction(filePath, error.line - 1, error.endLine - 1, preferred.index);
...
function buildSummary(intents: DetectedIntent[], propagated: PropagatedChange[], autoFixed: AutoFix[], remaining: RemainingError[]): string {
	const parts: string[] = [];
