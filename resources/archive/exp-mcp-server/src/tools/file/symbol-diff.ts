 ...
 */
export function diffSymbols(oldSymbols: NativeDocumentSymbol[], newSymbols: NativeDocumentSymbol[], editInfo?: EditInfo): DetectedIntent[] {
	...
	for (const [name, newSym] of newByName) {
		if (oldByName.has(name)) {
			...
			if (oldSym) {
				const adjustedNewRange = editInfo && newSym.range.startLine > editInfo.newContentEndLine ? shiftRange(newSym.range, -editInfo.linesDelta) : newSym.range;
 ...
 */
function diffChildren(oldChildren: NativeDocumentSymbol[], newChildren: NativeDocumentSymbol[], parentName: string): DetectedIntent[] {
