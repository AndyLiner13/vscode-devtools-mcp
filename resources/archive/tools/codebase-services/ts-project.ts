...
export function getWorkspaceProject(rootDir: string): Project {
	...
	if (tsConfigPath) {
		...
		const sourceGlobs = [path.join(rootDir, '**/*.ts'), path.join(rootDir, '**/*.tsx'), path.join(rootDir, '**/*.js'), path.join(rootDir, '**/*.jsx'), path.join(rootDir, '**/*.mts'), path.join(rootDir, '**/*.mjs')];
		try {
	...
	if (fs.existsSync(refConfigPath)) {
		try {
			...
			if (effectiveOptions.allowJs) {
				...
				if (!hasJsPatterns) {
					const jsPatterns = includePatterns.filter((p) => p.includes('.ts')).map((p) => p.replaceAll(/\.ts$/g, '.js').replaceAll('*.ts', '*.js'));
 ...
 */
function resolveExtendedCompilerOption(configPath: string, config: Record<string, unknown>): { allowJs: boolean; checkJs: boolean } {
