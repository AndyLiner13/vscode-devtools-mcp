...
function processSymbolNode(symbol: SymbolNode, ctx: ChunkContext, parentChunkId: null | string, breadcrumbParts: string[], depth: number): ProcessResult {
	...
	if (tokenCount > ctx.tokenBudget && childChunkIds.length === 0) {
		...
		const subChunks = splitOversizedChunk(ctx.lines, symbol, ctx.relativePath, symbolName, breadcrumbStr, depth, chunkId, ctx.tokenBudget);
		const subIds: string[] = [];
...
function generateChunkId(filePath: string, symbolName: string, range: { start: number; end: number }): string {
 ...
 */
function splitOversizedChunk(lines: string[], symbol: SymbolNode, filePath: string, symbolName: string, breadcrumb: string, depth: number, parentId: string, tokenBudget: number): Chunk[] {
