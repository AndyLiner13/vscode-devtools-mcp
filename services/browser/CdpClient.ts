...
export class CdpClient {
	...
	async send(method: string, params?: Record<string, unknown>, options?: CdpSendOptions): Promise<Record<string, unknown>> {
		...
		ws.on('message', (data: WebSocket.Data) => {
			...
			if ('id' in parsed && typeof parsed.id === 'number') {
				...
				if (pending) {
					...
					if (response.error) {
						pending.reject(new Error(`CDP error in ${pending.method}: ${response.error.message}`));
					} else {
