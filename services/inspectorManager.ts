...
async function startInspector(workspacePath: string): Promise<boolean> {
  ...
  if (!ready) {
    ...
    vscode.window.showWarningMessage(
      'MCP Inspector started but did not respond in time. It may still be loading — try opening http://localhost:6275 manually.',
    );
    return false;
  }
...
async function stopInspector(): Promise<boolean> {
  ...
  vscode.window.showWarningMessage(
    'MCP Inspector is running but was not started by this extension. Please stop it manually.',
  );
  return false;
}
