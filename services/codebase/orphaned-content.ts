 ...
 */
export function extractOrphanedContent(filePath: string, symbolRanges: Array<{ start: number; end: number }> = []): OrphanedContentResult {
...
function extractExports(sourceFile: SourceFile): SymbolNode[] {
	...
	for (const statement of sourceFile.getStatements()) {
		...
		const isDefault = 'hasDefaultKeyword' in statement && typeof statement.hasDefaultKeyword === 'function' && statement.hasDefaultKeyword();
...
function extractOrphanComments(sourceFile: SourceFile, symbolRanges: Array<{ start: number; end: number }>, imports: SymbolNode[], exports: SymbolNode[], directives: SymbolNode[]): SymbolNode[] {
	const result: SymbolNode[] = [];
...
function computeGaps(sourceFile: SourceFile, totalLines: number, coveredRanges: Array<{ start: number; end: number }>): Array<{ start: number; end: number; type: 'blank' | 'unknown' }> {
