...
export class ProcessLedger {
	...
	async refreshActiveChildren(): Promise<void> {
		...
		const allPids = [...Array.from(this.activeProcesses.keys()), ...Array.from(this.orphanedProcesses.keys())];
		if (allPids.length === 0) {
	 ...
	 */
	private findRootParent(child: ChildProcessInfo, allChildren: ChildProcessInfo[], rootPids: number[]): number {
	...
	private async queryChildProcessTree(rootPids: number[]): Promise<ChildProcessInfo[]> {
		...
		try {
			const { stdout } = await execAsync(`powershell.exe -NoProfile -NonInteractive -EncodedCommand ${encoded}`, { timeout: CHILD_QUERY_TIMEOUT_MS });
