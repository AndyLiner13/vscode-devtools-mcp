...
import { browserExecuteWithDiff, browserFetchAXTree, browserTypeIntoElement } from '../../host-pipe.js';
...
export const keyboardType = defineTool({
	...
	handler: async (request, response) => {
		const { uid, value, clear, includeSnapshot, response_format: format } = request.params;

		let changes: string | undefined;
		if (includeSnapshot) {
			await browserTypeIntoElement(uid, value, clear);
			const { formatted } = await browserFetchAXTree(false);
			changes = formatted;
		} else {
			const result = await browserExecuteWithDiff('type', { uid, value, clear });
