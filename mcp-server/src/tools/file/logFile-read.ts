...
export const /**
	...
	logFileRead = defineTool({
		...
		schema: {
			...
			includeStackFrames: zod.boolean().optional().describe('Show/hide stack frame templates in compressed logs. Default: true.'),
			minDuration: zod.string().optional().describe("Show log templates with durations >= threshold: '1s', '500ms', '100ms'."),
			pattern: zod.string().optional().describe('Regex pattern to filter log content (case-insensitive).'),
			severity: zod.enum(['error', 'warning', 'info']).optional().describe('Filter compressed logs by severity level.'),
			templateId: zod.string().optional().describe('Show raw lines matching this template ID from the compressed log overview.'),
			timeRange: zod.string().optional().describe("Time window filter for compressed logs: 'HH:MM-HH:MM' or 'HH:MM:SS-HH:MM:SS'.")
		}
	});
