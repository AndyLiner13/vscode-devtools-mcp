...
export const /**
	...
	lint = defineTool({
		...
		description:
			'Find dead code, unused exports, and code quality issues.\n\n' +
		...
		schema: {
			checks: zod
				.array(zod.enum(['all', 'dead-code', 'duplicates', 'errors', 'warnings', 'circular-deps']))
				.optional()
				.default(['all'])
				.describe("Which lint checks to run. Default: ['all']. " + "Available: 'dead-code', 'duplicates', 'errors', 'warnings', 'circular-deps'."),
			...
			limit: zod.number().int().min(1).max(500).optional().default(100).describe('Max results to return. Default: 100.')
		}
	});
