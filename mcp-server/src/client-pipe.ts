...
function assertResult<T extends object>(result: unknown, method: string): asserts result is T {
	if (typeof result !== 'object' || result === null) {
		throw new Error(`Invalid response from Client ${method}: expected object, got ${typeof result}`);
	}
}
...
async function sendClientRequest(method: string, params: Record<string, unknown>, timeoutMs: number = DEFAULT_TIMEOUT_MS): Promise<unknown> {
	return new Promise((resolve, reject) => {
		...
		client.on('data', (chunk: string) => {
			...
			if (nlIdx !== -1) {
				try {
					const rawParsed: unknown = JSON.parse(response.slice(0, nlIdx));
					if (!isJsonRpcResponse(rawParsed)) {
						settle(reject, new Error(`Invalid JSON-RPC response from Client ${method}`));
						return;
					}
					if (rawParsed.error) {
						logger(`[client-pipe] ${method} ✗ error: [${rawParsed.error.code}] ${rawParsed.error.message}`);
						settle(reject, new Error(`Client ${method} failed [${rawParsed.error.code}]: ${rawParsed.error.message}`));
					} else {
						logger(`[client-pipe] ${method} ✓ success`);
						settle(resolve, rawParsed.result);
					}
				} catch (e: unknown) {
					settle(reject, new Error(`Failed to parse Client response: ${e instanceof Error ? e.message : String(e)}`));
				}
			}
		});
		...
		client.on('close', () => {
			settle(reject, new Error(`Client ${method} socket closed before response was received`));
		});

		const timer = setTimeout(() => {
			logger(`[client-pipe] ${method} ✗ TIMEOUT after ${timeoutMs}ms`);
			settle(reject, new Error(`Client ${method} request timed out (${timeoutMs}ms)`));
		}, timeoutMs);
	});
}
 ...
 */
export async function fileReadContent(filePath: string, startLine?: number, endLine?: number): Promise<FileReadContentResult> {
	const result = await sendClientRequest('file.readContent', { endLine, filePath, startLine }, 10_000);
 ...
 */
export async function fileFindReferences(filePath: string, line: number, character: number): Promise<FileFindReferencesResult> {
	const result = await sendClientRequest('file.findReferences', { character, filePath, line }, 10_000);
